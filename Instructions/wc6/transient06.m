...
% variables
global x x_u y y_v u v pc rho mu b SMAX SAVG aP aE aW aN aS eps k...
    u_old v_old pc_old Dt eps_old k_old NhI NhJ h uplus yplus yplus1 yplus2
% constants
global NPI NPJ XMAX YMAX LARGE U_IN SMALL Cmu sigmak sigmaeps C1eps C2eps kappa ERough Ti

NPI        = 250;       % number of grid cells in x-direction [-]
NPJ        = 30;        % number of grid cells in y-direction [-]
h          = 0.02;      % step height for BACKWARD-FACING STEP FLOW problm [m]
XMAX       = 25*h;      % width of the domain [m]
YMAX       = 3*h;      % height of the domain [m]
NhJ       = ceil(h/(YMAX/NPJ));    %y-index of step height [-]
NhI       = ceil(12.5*h/XMAX*NPI); %x-index of step height [-]
...
U_IN       = 0.25;      % inflow velocity [m/s]

% parameters for k-epsilon model
Cmu        = 0.09;
sigmak     = 1.;
sigmaeps   = 1.3;
C1eps      = 1.44;
...
Dt = 0.001;              % timestep [s], first estimated CFL number <1
TOTAL_TIME = 3.;         % total time when the simulation finishes [s]
...
for time = Dt:Dt:TOTAL_TIME
    ...
    if time == Dt
        fprintf ('Iter\t Time\t u\t v\t SMAX\t SAVG\n');
    end    
    fprintf ("%4d %10.3e\t%10.2e\t%10.2e\t%10.2e\t%10.2e\t%10.2e\t%10.2e\n",iter,...
        time,u(NhI+10,NhJ-5),v(NhI+10,NhJ-5),...
         u(NPI-10,NhJ-5),v(NPI-10,NhJ-5),SMAX, SAVG);
...
for I = 1:NPI+1
    ...
    for J = 2:NPJ+1
        ...
        fprintf(fp,'%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\n',...
            x(I), y(J), ugrid, vgrid, pc(I,J), rho(I,J), mu(I,J),  ...
...
figure
contourf(X,Y,u'/U_IN, 100,'LineColor','none'); % 100 levels, no contour lines
colormap(jet)                        % change as desired (parula, hot, etc.)
cb = colorbar('southoutside');
cb.Ticks = sort([cb.Limits, cb.Ticks]);
cb.Ruler.TickLabelFormat = '%.2f'; 
title('Streamwise velocity components along the channel')
% hold on
% quiver(X,Y,u',v',1);
axis equal
xlabel('x'), ylabel('y')
...
figure
contourf(X,Y,v'/U_IN, 100,'LineColor','none'); % 100 levels, no contour lines
colormap(jet)                        % change as desired (parula, hot, etc.)
cb = colorbar('southoutside');
cb.Ticks = sort([cb.Limits, cb.Ticks]);
cb.Ruler.TickLabelFormat = '%.2f'; 
title('Cross-streamwise velocity components along the channel')
axis equal
xlabel('x'), ylabel('y')

figure
contourf(X,Y,sqrt(u'.*u'+v'.*v')/U_IN, 30,'LineColor','black'); % 30 levels, no contour lines
colormap(jet)                        % change as desired (parula, hot, etc.)
cb = colorbar('southoutside');
cb.Ruler.TickLabelFormat = '%.2f'; 
title('Velocity magnitude values with streamline patterns along the channel')
axis equal
xlabel('x'), ylabel('y')