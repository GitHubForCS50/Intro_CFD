...
% variables
global y rho k tw yplus yplus1 yplus2 yplus3 uplus mu u NhJ NhI
...
for I = 1:NhI+1
    i = I;
    if yplus3(I,NhJ+1) < 11.63
        tw(I,NhJ+1)      = mu(I,NhJ+1)*0.5*(u(i,NhJ+1)+u(i+1,NhJ+1))/(y(NhJ+1)-y(NhJ));
        yplus3(I,NhJ+1)  = sqrt(rho(I,NhJ+1)*abs(tw(I,NhJ+1)))*(y(NhJ+1)-y(NhJ))/mu(I,NhJ+1);
        yplus(I,NhJ+1)   = yplus3(I,NhJ+1);
        uplus(I,NhJ+1)   = yplus(I,NhJ+1);
    else
        tw(I,NhJ+1)      = rho(I,NhJ+1)*Cmu^0.25*sqrt(k(I,NhJ+1))*0.5*(u(i,NhJ+1)+u(i+1,NhJ+1))/uplus(I,NhJ+1);
        yplus3(I,NhJ+1)  = sqrt(rho(I,NhJ+1)*abs(tw(I,NhJ+1)))*(y(NhJ+1)-y(NhJ))/mu(I,NhJ+1);
        yplus(I,NhJ+1)   = yplus3(I,NhJ+1);
        uplus(I,NhJ+1)   = log(ERough*yplus(I,NhJ+1))/kappa;
    end
end
