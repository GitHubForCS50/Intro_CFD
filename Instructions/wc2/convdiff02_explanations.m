...
%% constants
NPI         = 66;      % number of grid cells in x-direction [-]
NPJ         = 66;      % number of grid cells in y-direction [-]
...
rho = zeros(NPI+2,NPJ+2);   % density [kg/m3]
...
% a   = zeros(NPI+2,NPJ+2);   % const. vol. source term [W/m3]
% b_s = zeros(NPI+2,NPJ+2);   % linear. coeff. for source term [W/Km3]
...
% This is the coordinates at the boundaries, not in the centers, because we
% need velocities at boundaries.
% When calling x(3), we get 3rd point. When calling x_u(3) we get the
% boundary west (left) of 3rd point!
x_u(1) = 0.;
x_u(2) = 0.;
for i = 3:NPI+2
    x_u(i) = x_u(i-1) + Dx;
...
for iter = 1:OUTER_ITER
    ...
    for I = 2:NPI+2
        i = I; % i is now for boundaries and I is for points. i is thus used for x_u.
        for J = 2:NPJ+2
            j = J;
            % This is flux on west side of point I in x-dir for the row J
            % x(I)-x_u(i) is just Dx/2, also x_u(i)-x(I-1) = Dx/2
            % x(I)-x(I-1) is Dx
            % So the expression is kind of a lever rule and is used if grid
            % is not uniform. rho(I-1,J) at point west of current is more
            % important if the boundary is close to point w a.k.a.
            % x(I)-x_u(i) is large.
            F_u(i,J) = (rho(I-1,J)*(x(I)-x_u(i)) + rho(I,J)*(x_u(i)- x(I-1)))*u(i,J)/(x(I)-x(I-1)); % = F(i,j)
            % This is flux on south side of point J in y-dir for the column I
