...
% variables
global x x_u y y_v v p mu SP Su F_u F_v d_v relax_v v_old rho Istart Iend ...
    Jstart Jend b aE aW aN aS aP dudy dvdy
...
for I = Istart:Iend
    ...
    for J = Jstart:Jend
        ...
        % The source terms
        mue = 0.25*(mu(I,J-1)+mu(I+1,J-1)+mu(I+1,J)+mu(I,J));
        muw = 0.25*(mu(I,J)+mu(I,J-1)+mu(I-1,J-1)+mu(I-1,J));
        SP(I,j) = 0.;
        Su(I,j) = ((mu(I,J)*dvdy(I,J)-mu(I,J-1)*dvdy(I,J-1))/(y(J)-y(J-1)) ...
                  + (mue*dudy(i+1,j)-muw*dudy(i,j))/(x_u(i+1)-x_u(i)))*AREAw*AREAs;
