...
% variables
global x x_u y y_v u v pc T rho mu Gamma b SMAX SAVG aP aE aW aN aS ...
    u_old v_old pc_old T_old Dt 
% constants
...
run_dir = fullfile('outputs', timestamp); 
mkdir(run_dir);
...
for time = times
    ...
    us(i) = u(ceil(3*(NPI+1)/10),ceil(2*(NPJ+1)/5));
    vs(i) = v(ceil(3*(NPI+1)/10),ceil(2*(NPJ+1)/5));
    Ts(i) = T(ceil(3*(NPI+1)/10),ceil(2*(NPJ+1)/5));
...
%% begin: output()
fosc = fopen(fullfile(run_dir,'osc.dat'),"w");
fprintf(fosc,'step\ttime\tu\tv\tT\n');
for i = 1:length(times)
    fprintf(fosc,'%i\t%11.5e\t%11.5e\t%11.5e\t%11.5e\n',i,times(i),us(i),vs(i),Ts(i));
end
fclose(fosc);

% print all results in output.txt
fp = fopen(fullfile(run_dir,'output.dat'),'w');
for I = 1:NPI+1
    i = I;
    for J = 2:NPJ+1
        j = J;
...
vort = fopen(fullfile(run_dir,'vort.dat'),'w');
for I = 2:NPI+1
    i = I;
    for J = 2:NPJ+1
        j = J;
...
str = fopen(fullfile(run_dir,'str.dat'),'w');
for I = 1:NPI+1
    i = I;
    for J = 1:NPJ+1
        j = J;
...
velu = fopen(fullfile(run_dir,'velu.dat'),'w');
for I = 2:NPI+2
    i = I;
    for J = 1:NPJ+2
...
velv = fopen(fullfile(run_dir,'velv.dat'),'w');
for I = 1:NPI+2
    for J = 2:NPJ+2
        j = J;
...
quiver(X,Y, u', v',5);
hold on;
x_idx = ceil(3*(NPI+1)/10);
y_idx = ceil(2*(NPJ+1)/5);
x_point = x(x_idx);
y_point = y(y_idx);
