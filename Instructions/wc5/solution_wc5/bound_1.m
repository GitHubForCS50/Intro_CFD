...
% constants
global NPI NPJ U_IN YMAX Cmu Ti JinLow JinHigh
% variables
global y u v T m_in m_out y_v F_u k eps f
...
for J = 1:NPJ+2
    if J >=round(JinLow*NPJ) && J<=round(JinHigh*NPJ)
        f(1,J) = 1.0;
    else 
        f(1,J) = 0.0;
    end
...
f(1:NPI+2,1)=f(1:NPI+2,2);
f(1:NPI+2,NPJ+2)=f(1:NPI+2,NPJ+1);
...
f(NPI+2,1:NPJ+2) = f(NPI+1,1:NPJ+2);
end
